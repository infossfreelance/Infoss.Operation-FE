...
const InvoicePage = () => {
  ...
  const fetchInvoices = (pageNumber = 1, pageSize = 10, filter = []) => {
    ...
    axios
      .post(
        API_URL +
          `invoice/invoice/PostByPage?pageNumber=${pageNumber}&pageSize=${pageSize}`,
        payload
      )
      .then((response) => {
  ...
  const handleEdit = () => {
    if (SelectedData.id === undefined) {
      ...
      if (SelectedData.rowStatus === 'DEL') {
        ...
        SelectedData.sfpReference !== null ||
        SelectedData.sfpReference.length > 0
      ) {
      ...
      return (
        <div style={{display: 'inline-block'}}>
          <Pagination>
            <Pagination.Prev
              onClick={() => {
                fetchInvoices(numPage - 1, rowsCount);
                setNumPage(numPage - 1);
              }}
            />
            <Pagination.Item active>{numPage}</Pagination.Item>
  ...
  const handleSaveModal = (payload) => {
    ...
    if (modalType === 'credit') {
      ...
      axios
        .put('https://localhost:7160/Invoice/UpdateStatusDelivered', body)
        .then((response) => {
          setLoading(false);
          if (response.data.code === 200) {
            fetchInvoices(numPage, rowsCount);
        ...
        <Grid item xs={10}>
          <Stack direction="row" spacing={1}>
            <Button
              ...
              className="btn-sm"
              onClick={() => fetchInvoices(numPage, rowsCount)}
            >
            ...
            <Button
              variant="outline-infoss"
              className="btn-sm"
              onClick={() => handleOpenModalDelivered()}
            >
