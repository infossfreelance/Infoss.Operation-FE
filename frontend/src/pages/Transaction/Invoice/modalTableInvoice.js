...
const ModalTableInvoice = (props) => {
      ...
      if (props.type === 'shipment') {
        ...
        axios
          ...
          .then((res) => {
            if (res.data.code === 200) {
              let temp =
                res.data.data.estimateProfitLoss.estimateProfitLossDetails;
      ...
      return (
        <div style={{display: 'inline-block'}}>
          <Pagination>
            <Pagination.Prev
              onClick={() => {
                props.fetchData(rowsCount, numPage - 1);
                setNumPage(numPage - 1);
              }}
            />
            <Pagination.Item active>{numPage}</Pagination.Item>
  ...
  const renderTitle = () => {
    if (props.type === 'shipment') {
      ...
      return <h4>List Revised Tax Invoice</h4>;
    } else if (props.type === 'storage') {
      return <h4>List Storage</h4>;
    } else if (props.type === 'hf') {
      return <h4>List Handling Fee</h4>;
    } else {
      return <h4>List Profit Share</h4>;
    }
  };
