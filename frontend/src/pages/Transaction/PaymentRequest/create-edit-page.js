import React, { useEffect, useState, Dispatch, useSyncExternalStore } from 'react';
...
const CrudPaymentRequestPage = () => {
  ...
  const [stateBeingForPaymentByCustomer, setStateBeingForPaymentCustomer] = useState([])
  const [newArrPaymentReqeustDetails, setNewArrPaymentReqeutDetails] = useState([])
  ...
  const getCustomerByCustomerTypeId = (rowStatus = 50, NumPage = 1, shipmentId, customerId = 0, stateCustomerType, isIncome = false) => {
    if (shipmentId && stateCustomerType) {
      axios.post(`http://stage-operation.api.infoss.solusisentraldata.com/estimateProfitLossV1/estimateProfitLoss/PostBySearchCustomerEPL?pageNumber=${NumPage}&pageSize=${rowStatus}&eplId=${shipmentId}&customerId=${customerId}&customerTypeId=${stateCustomerType}&isIncome=${isIncome}`, {
        userCode: 'luna',
        countryId: 101,
        companyId: 32,
        branchId: 12,
  ...
  const getBeingForPaymentByCustomer = (rowStatus = 50, NumPage = 1, shipmentId, customerId, stateCustomerType = 0, isIncome = false) => { 
    if (shipmentId && customerId) { 
      axios.post(`http://stage-operation.api.infoss.solusisentraldata.com/estimateProfitLossV1/estimateProfitLoss/PostBySearchCustomerEPL?pageNumber=${NumPage}&pageSize=${rowStatus}&eplId=${shipmentId}&customerId=${customerId}&customerTypeId=${stateCustomerType}&isIncome=${isIncome}`, {
        ...
        if (response.status === 200) { 
          setStateBeingForPaymentCustomer(response?.data?.data?.estimateProfitLossDetail)
        }
      }).catch((err) => {
        console.log(err)
      })
  ...
  useEffect(() => {
    if (shipmentId && stateCustomerType) {
      // const getCustomerByCustomerTypeId = (rowStatus = 50, NumPage = 1, shipmentId, customerId = 0, stateCustomerType, isIncome = false) => {
      getCustomerByCustomerTypeId(50, 1, shipmentId, 0, stateCustomerType, false)
    }
  }, [shipmentId, stateCustomerType])

  useEffect(() => { 
    if (shipmentId && customerId) { 
      getBeingForPaymentByCustomer(50, 1, shipmentId, customerId, 0, false)
    }
  ...
  const handleCustomer = () => {
    if (!shipmentNo && !shipmentId && shipmentNo !== '-') {
      ...
      console.log(stateBeingForPaymentByCustomer, '<<<stateBeingForPaymentByCustomer')
      ...
      const data =   tempArray.map((ele) => { 
          ...
          tempPayload['fakturDate'] = ele?.fakturDate || "2022-11-20T15:12:02.333Z"
          ...
          tempPayload['deliveryDate'] = ele?.deliveryDate || "2022-11-20T15:12:02.332Z"
        ...
        setNewArrPaymentReqeutDetails(...data)
        console.log(data, '<<<Data')
      // setArrPayment(data, newArrPaymentReqeustDetails)
      }
    }
  }
      ...
      <Grid container spacing={2} direction="column">
        ...
        <Paper variant="outlined" sx={{ m: 2, p: 2 }}>
          ...
          <ModalCustomer
            ...
            fetchDataCustomer={(r, p, si, sct, ci, boo) => getCustomerByCustomerTypeId(r, p, si, sct, ci, boo)}
            ...
            <Grid item xs={4}>
              ...
              <div className="row mt-3 mx-3">
                <TextField
                  ...
                  //   !isEditDisabled ? setOpenMLSO(true) : setOpenMLSO(false)
                  // }
                  value={etd?.length > 0 ? dateFormat(etd) : dateFormat('01-01-1953')}
              ...
              <div className="row">
                <div className="col-8">
                  <TextField
                    ...
                    size="small"
                    onChange={(e) => {
                      console.log(e, '<<eventShipmentNomber')
                      if (e === null || e.target.value === undefined || e.target.value === '' || e.target.value === '-') {
                ...
                <Table sx={{ minWidth: 650 }} aria-label="simple table">
                  <TableHead>
                    <TableRow>
                      ...
                      <TableCell>NoPol Truck.</TableCell>
                      <TableCell>Driver</TableCell>
                    </TableRow>
                  </TableHead>
                  <TableBody>
                    {console.log(newArrPaymentReqeustDetails, '<<<newArrPaymentReqeustDetails')}
